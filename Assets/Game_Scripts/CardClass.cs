...
namespace GwentPro
{
    public class CardClass : MonoBehaviour
    {
        ...
        EndTurnButton end;
        public Sprite def;
        public TextMeshProUGUI Lureinfo;
        public GameObject LureinfoInstance;
        ...
        void Start()
        {
            ...
            if (
                gameObject.name == "Aleyra Valiente"
                || gameObject.name == "Fernás"
                || gameObject.name == "Profesor R"
                || gameObject.name == "Idris Anhia"
            )
            {
        ...
        void OnMouseEnter()
        {
            if (ActualScene == "GameTable")
            {
                ...
                Zoom.GetComponent<SpriteRenderer>().sprite = gameObject
                ...
                else if(cmbtype == combatype.Increase)
                {
                    Zoom.transform.localScale = new Vector3(0.28f, 0.4f, 0.2f);
                }
                else
                {
        ...
        void OnMouseDown()
        {
            selected = !selected;
            if (
                cmbtype == combatype.Lure
                && ActualScene != "RedrawScene"
                && ActualScene != "RedrawSuns"
            )
            {
                    if (gameObject.tag == "Crows")
                    {
                        if (selected)
                        {
                            ...
                            LureinfoInstance = Instantiate(Lureinfo, canvas.transform, false).gameObject;
                            EndTurnButton checkturn = GameObject
                                .Find("EndTurnButton")
                                .GetComponent<EndTurnButton>();
                            bool CrTurn = checkturn.CrTurn;
                            LureinfoInstance.GetComponent<TextMeshProUGUI>().text = "Selecciona la carta que quieres intercambiar";
                            LureinfoInstance.transform.rotation = Quaternion.Euler(0, 0, 0);
                        }
                        else
                        {
                            Vector3 newPosition = new Vector3(
                            ...
                            transform.position = newPosition;
                            if (LureinfoInstance != null)
                            {
                                Destroy(LureinfoInstance);
                            }
                        }
                    }
                ...
                else
                {
                    if (selected)
                    {
                        ...
                        bool CrTurn = checkturn.CrTurn;
                        LureinfoInstance.GetComponent<TextMeshProUGUI>().text = "Selecciona la carta que quieres intercambiar";
                        LureinfoInstance.transform.rotation = Quaternion.Euler(180, 180, 0);
                    }
                    else
                    {
                        Vector3 newPosition = new Vector3(
        ...
        void OnCollisionEnter2D(Collision2D collision)
        {
            ...
            HorizontalLayoutGroup layoutGroup =
                collision.gameObject.GetComponent<HorizontalLayoutGroup>();
